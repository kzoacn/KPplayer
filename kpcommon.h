...
static inline unsigned char prevent_byte_overflow(int x) {
    if(x > 255) {
        g_printerr("kp: [warning] clipping %d to 255\n", x);
        return 255;
    }
    if(x < 0) {
        g_printerr("kp: [warning] clipping %d to 0\n", x);
        return 0;
    }
    return x;
}
...
static inline unsigned char prevent_yuv_overflow(int x) {
    if(x > 235) {
        g_printerr("kp: [warning] clipping %d to 235\n", x);
        return 235;
    }
    if(x < 16) {
        g_printerr("kp: [warning] clipping %d to 16\n", x);
        return 16;
    }
    return x;
}
